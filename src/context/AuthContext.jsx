...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/users/me`, {
  ...
  const loginEmail = async (email, password) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/auth/login`, {
      method: 'POST',
  ...
  const registerEmail = async (email, password) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },

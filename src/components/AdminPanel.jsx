...
const AdminPanel = ({ products, setProducts, posts, setPosts }) => {
  ...
  const handleSaveProduct = async (productData) => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/products`, {
        method: 'POST',
  ...
  const handleDeleteProduct = async (id) => {
    if (window.confirm('Bạn có chắc chắn muốn xoá sản phẩm này?')) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/products/${id}`, { method: 'DELETE' });
  ...
  const handleSavePost = async (postData) => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/posts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updatedData)
      });
  ...
  const handleDeletePost = async (id) => {
    if (window.confirm('Bạn có chắc chắn muốn xoá bài viết này?')) {
       try {
         await fetch(`${import.meta.env.VITE_API_URL || 'https://better-grg6.onrender.com'}/api/posts/${id}`, { method: 'DELETE' });
